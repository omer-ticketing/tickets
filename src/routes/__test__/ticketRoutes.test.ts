...
describe("Tickets fetching", () => {
    ...
    it("Returns the ticket if the ticket is found.", async () => {
        ...
        const ticketResponse = await request(app)
            .get(`/api/tickets/${response.body.data.ticket.id}`)
            .send()
            .expect(200);
...
describe("Ticket update", () => {
    it("Returns a 401 if the user is not authenticated.", async () => {
        await request(app).patch(`/api/tickets/${new ObjectId().toString()}`).send({ price: 33 }).expect(401);
    });
    ...
    it("Returns a 400 if the user updates the ticket and provided an invalid title or price.", async () => {
        const cookie = getAuthCookie();
        ...
        await request(app).patch(`/api/tickets/${ticketId}`).set("Cookie", cookie).send({ price: "a34" }).expect(400);
    });
    ...
    it("Rejects the ticket update if it is reserved", async () => {
        const cookie = getAuthCookie();
        const ticketResponse = await request(app)
            .post("/api/tickets")
            .set("Cookie", cookie)
        ...
        ticket!.set({ orderId: new ObjectId().toString() });
        await ticket!.save();
